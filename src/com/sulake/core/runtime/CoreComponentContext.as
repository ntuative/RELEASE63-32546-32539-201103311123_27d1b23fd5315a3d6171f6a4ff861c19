package com.sulake.core.runtime
{
   ...
   public final class CoreComponentContext extends ComponentContext implements ICore
   {
      ...
      private static const const_1351:String = "asset-libraries";
      
      private static const const_1347:String = "library";
      
      private static const const_1348:String = "service-libraries";
      
      private static const const_1350:String = "library";
      
      private static const const_1345:String = "component-libraries";
      
      private static const const_1352:String = "library";
      
      private static const const_1349:String = "error_data";
      
      private static const const_1353:String = "error_cat";
      
      private static const const_1346:String = "error_desc";
       
      
      private var var_367:LibraryLoaderQueue;
      
      private var var_765:IEventDispatcher;
      
      private var var_517:uint;
      
      private var var_368:Function;
      
      private var var_204:Profiler;
      ...
      public function CoreComponentContext(param1:DisplayObjectContainer, param2:uint)
      {
         ...
         switch(param2 & 0)
         {
            case Core.const_1140:
               debug("Core; using simple frame update handler");
               this.var_368 = this.simpleFrameUpdateHandler;
               break;
            case Core.const_908:
               debug("Core; using complex frame update handler");
               this.var_368 = this.complexFrameUpdateHandler;
               break;
            case Core.const_797:
               debug("Core; using profiler frame update handler");
               this.var_368 = this.profilerFrameUpdateHandler;
               this.var_204 = new Profiler(this);
               attachComponent(this.var_204,[new IIDProfiler()]);
               var_403.addChild(new ProfilerViewer(this.var_204));
               break;
            case Core.const_590:
               debug("Core; using experimental frame update handler");
               this.var_368 = this.experimentalFrameUpdateHandler;
               break;
            case Core.CORE_SETUP_DEBUG:
               debug("Core; using debug frame update handler");
               this.var_368 = this.debugFrameUpdateHandler;
         }
      }
      
      public function getNumberOfFilesPending() : uint
      {
         return this.var_367.length;
      }
      
      public function getNumberOfFilesLoaded() : uint
      {
         return this.var_517 - this.getNumberOfFilesPending();
      }
      ...
      override public function dispose() : void
      {
         ...
         if(!disposed)
         {
            ...
            {
               i = 0;
               while(i < CoreComponentContext.const_102)
               {
                  receivers = this.var_259[i] as Array;
      ...
      override public function error(param1:String, param2:Boolean, param3:int = -1, param4:Error = null) : void
      {
         if(param4)
         {
            ErrorReportStorage.setParameter(const_1349,String(param4.getStackTrace()));
         }
         ErrorReportStorage.setParameter(const_1353,String(param3));
         ErrorReportStorage.setParameter(const_1346,param1);
      ...
      public function readConfigDocument(param1:XML, param2:IEventDispatcher = null) : void
      {
         ...
         this.var_765 = loadingEventDelegate;
         if(this.var_367 == null)
         {
            this.var_367 = new LibraryLoaderQueue(_debug);
         }
         try
         {
            node = config.child(const_1351)[0];
            if(node != null)
            {
               list = node.child(const_1347);
         ...
         try
         {
            node = config.child(const_1348)[0];
            if(node != null)
            {
               list = node.child(const_1350);
               for each(item in list)
               {
                  url = item.attribute("url").toString();
                  loader = new LibraryLoader(context,true,_debug);
         ...
         try
         {
            node = config.child(const_1345)[0];
            if(node != null)
            {
      ...
      private function errorInLoadingProcess(param1:LibraryLoaderEvent = null) : void
      {
         ...
         this.error("Failed to download library \"" + _loc2_.url + "\" HTTP status " + param1.status + ": " + _loc2_.getLastErrorMessage(),true,Core.const_1484);
         if(!disposed)
         {
      ...
      private function finalizeLoadingEventDelegate() : void
      {
         if(this.var_765)
         {
            this.var_765.dispatchEvent(new Event(Event.COMPLETE));
         }
      }
      ...
      private function updateLoadingProgress(param1:LibraryLoaderEvent = null) : void
      {
         ...
         if(this.var_765)
         {
            ...
            this.var_765.dispatchEvent(new LibraryProgressEvent(_loc2_.url,param1.bytesLoaded,param1.bytesTotal,_loc2_.elapsedTime));
         }
      }
         ...
         if(param1 != null)
         {
            if(param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_COMPLETE || param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_ERROR)
            {
               ...
               if(!disposed)
               {
                  if(this.var_765)
                  {
                     this.var_765.dispatchEvent(new LibraryProgressEvent(_loc2_.url,this.var_517 - this.var_367.length,this.var_517,_loc2_.elapsedTime));
                  }
               }
            }
         }
      ...
      override public function registerUpdateReceiver(param1:IUpdateReceiver, param2:uint) : void
      {
         ...
         var _loc3_:int = !!this.var_204 ? 0 : this.var_1453 & 0;
         if(_loc3_ == Core.const_590)
         {
      ...
      private function onEnterFrame(param1:Event) : void
      {
         ...
         this.var_368(_loc2_,_loc2_ - this._lastUpdateTimeMs);
      ...
      private function complexFrameUpdateHandler(param1:uint, param2:uint) : void
      {
         ...
         while(priority < CoreComponentContext.const_102)
         {
            ...
            {
               if(this.var_623[priority] < priority)
               {
                  ++this.var_623[priority];
                  skip = true;
               }
            }
            if(!skip)
         ...
         while(priority < CoreComponentContext.const_102)
         {
            ...
            while(index != length)
            {
               ...
               else
               {
                  try
                  {
                     this.var_204.update(receiver,msSinceLastUpdate);
                  }
                  catch(e:Error)
                  {
                     error("Error in update receiver \"" + getQualifiedClassName(receiver) + "\": " + e.message,true,e.errorID,e);
      ...
      public function setProfilerMode(param1:Boolean) : void
      {
         ...
         else
         {
            if(this.var_204)
            {
               detachComponent(this.var_204);
               this.var_204.dispose();
               this.var_204 = null;
            }
            switch(this.var_1453 & 0)
            {
               case Core.const_1140:
                  this.var_368 = this.simpleFrameUpdateHandler;
                  break;
