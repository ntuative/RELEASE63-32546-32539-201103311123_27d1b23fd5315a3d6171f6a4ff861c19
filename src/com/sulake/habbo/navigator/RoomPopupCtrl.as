package com.sulake.habbo.navigator
{
   ...
   public class RoomPopupCtrl extends PopupCtrl
   {
      ...
      public function RoomPopupCtrl(param1:HabboNavigator, param2:int, param3:int)
      {
         ...
         this.var_2188 = new RoomThumbnailRenderer(param1);
         this.var_1232 = new TagRenderer(param1,method_3);
      }
      ...
      override public function refreshContent(param1:IWindowContainer) : void
      {
         ...
         {
            this.var_180 = IWindowContainer(param1.findChildByName("details_container"));
         }
         this.var_180.visible = true;
         Util.hideChildren(this.var_180);
         this.refreshRoomName(this.var_180,this.var_113);
         if(this.var_113.ownerName != "" && this.var_113.ownerName != "-")
         {
         }
         this.refreshOwnerName();
         this.refreshTextWithCaption("roomctg",this.var_180,!!this.var_113.event ? this.getEventCtg(this.var_113.categoryId) : this.getRoomCtg(this.var_113.categoryId));
         this.refreshRoomDesc(this.var_180,this.var_113);
         this.refreshExtraCont();
         this.refreshInfo(this.var_180,"trading_allowed",this.var_113.allowTrading);
         this.refreshInfo(this.var_180,"doormode_doorbell",this.var_113.doorMode == 1);
         this.refreshInfo(this.var_180,"doormode_password",this.var_113.doorMode == 2);
         Util.moveChildrenToColumn(this.var_180,["roomname","roomctg_cont","roomowner_cont","roomdesc","extra_cont","doormode_doorbell","doormode_password","trading_allowed"],0,0);
         this.var_180.height = Util.getLowestPoint(this.var_180);
      }
      
      private function refreshOwnerName() : void
      {
         this.refreshTextWithCaption("roomowner",this.var_180,this.var_113.ownerName);
         var _loc1_:ITextWindow = ITextWindow(this.var_180.findChildByName("roomowner"));
         if(this.var_113.ownerName != "" && this.var_113.ownerName != "-")
         {
      ...
      {
         var _loc1_:IWindowContainer = IWindowContainer(this.var_180.findChildByName("extra_cont"));
         Util.hideChildren(_loc1_);
         this.var_1232.refreshTags(_loc1_,this.var_113.tags);
         ...
         this.var_2188.refreshThumbnail(_loc1_,this.var_113.thumbnail,false);
      ...
      private function onOwnerName(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         if(_loc3_ != null)
         {
            navigator.mainViewCtrl.startSearch(Tabs.const_266,Tabs.const_233,_loc3_.text);
            this.method_3();
         }
      }
   }
