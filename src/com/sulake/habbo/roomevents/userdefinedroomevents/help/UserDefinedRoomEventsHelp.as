package com.sulake.habbo.roomevents.userdefinedroomevents.help
{
   ...
   public class UserDefinedRoomEventsHelp
   {
      ...
      private function prepareWindow() : void
      {
         ...
         this.var_1838 = IWindowContainer(this.find(this._window,"help_container"));
         this.var_1193 = this.var_1838.findChildByName("button_previous") as IContainerButtonWindow;
         if(this.var_1193)
         {
            this.var_1193.procedure = this.onButtonClick;
         }
         this.var_1194 = this.var_1838.findChildByName("button_next") as IContainerButtonWindow;
         if(this.var_1194)
         {
            this.var_1194.procedure = this.onButtonClick;
         }
      ...
      {
         if(this.var_263 >= this.var_69.length - 1)
         {
            this.var_1194.disable();
         }
         else
         {
            this.var_1194.enable();
         }
         if(this.var_263 <= 0)
         {
            this.var_1193.disable();
         }
         else
         {
            this.var_1193.enable();
         }
      }
      ...
      {
         ++this.var_263;
         this.var_263 = Math.min(this.var_263,this.var_69.length - 1);
         this.openPage();
      }
      ...
      {
         --this.var_263;
         this.var_263 = Math.max(this.var_263,0);
         this.openPage();
      }
      ...
      private function initLocalizations() : void
      {
         ...
         while(this.var_81.localization.getLocalization("wiredfurni.help." + _loc1_ + ".title"))
         {
            ...
            this.var_69.push([_loc2_,_loc3_,_loc4_]);
            _loc1_++;
         }
      }
      ...
      private function localize() : void
      {
         ...
         if(!this.var_69 || this.var_69.length == 0)
         {
            return;
         }
         if(_loc1_)
         {
            _loc1_.text = ILocalization(this.var_69[this.var_263][0]).value;
         }
         if(_loc2_)
         {
            _loc2_.text = ILocalization(this.var_69[this.var_263][2]).value;
         }
         if(_loc3_)
         {
            _loc4_ = ILocalization(this.var_69[this.var_263][1]).value;
