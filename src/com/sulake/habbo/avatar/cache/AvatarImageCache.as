package com.sulake.habbo.avatar.cache
{
   ...
   public class AvatarImageCache
   {
      ...
      private static const const_1768:int = 60000;
      ...
      private var var_348:IActiveActionData;
      
      private var var_433:AvatarImageBodyPartContainer;
      ...
      private var var_2318:ByteArray;
      
      private var var_2319:ImageData;
      
      private var var_995:PaletteMap;
      
      private var var_900:Array;
      
      private var var_899:Point;
      ...
      public function setGeometryType(param1:String) : void
      {
         ...
         if(this.var_732 == GeometryType.const_1219 && param1 == GeometryType.const_1148 || this.var_732 == GeometryType.const_1148 && param1 == GeometryType.const_1219)
         {
            this.var_732 = param1;
            this.var_84 = null;
            return;
      ...
      public function getImageContainer(param1:String, param2:int) : AvatarImageBodyPartContainer
      {
         ...
         this.var_348 = this._bodyPartCache.getAction();
         var _loc5_:IActiveActionData = this.var_348;
         var _loc6_:* = [];
         var _loc7_:Dictionary = new Dictionary();
         var _loc8_:Point = new Point();
         if(!(!this.var_348 || !this.var_348.definition))
         {
            if(this.var_348.definition.isAnimation)
            {
               _loc11_ = _loc3_;
               _loc12_ = this._structure.getAnimation(this.var_348.definition.state + "." + this.var_348.actionParameter);
               _loc13_ = param2 - this.var_348.startFrame;
               if(_loc12_ != null)
               {
                  _loc14_ = _loc12_.getLayerData(_loc13_,param1,this.var_348.overridingAction);
                  if(_loc14_ != null)
                  {
                     ...
                     {
                        this.var_348 = _loc14_.action;
                     }
                     if(_loc14_.type == AnimationLayerData.const_692)
                     {
                        if(_loc14_.action != null)
                        {
                     ...
                     else if(_loc14_.type == AnimationLayerData.const_510)
                     {
                        _loc3_ = _loc11_;
                     }
         ...
         if(_loc10_ == null)
         {
            _loc15_ = this._structure.getParts(param1,this.var_1371.getFigure(),_loc5_,this.var_732,_loc3_,_loc6_,_loc7_);
            _loc10_ = new AvatarImageDirectionCache(_loc15_);
            _loc9_.updateDirectionCache(_loc3_,_loc10_);
         }
         this.var_433 = _loc10_.getImageContainer(_loc4_);
         if(this.var_433 == null)
         {
            _loc16_ = _loc10_.getPartList();
            this.var_433 = this.renderBodyPart(_loc3_,_loc16_,_loc4_,this.var_348);
            if(this.var_433 == null)
            {
               return null;
            }
            _loc10_.updateImageContainer(this.var_433,_loc4_);
         }
         this.var_433.offset = _loc8_;
         return this.var_433;
      }
      ...
      private function renderBodyPart(param1:int, param2:Array, param3:int, param4:IActiveActionData) : AvatarImageBodyPartContainer
      {
         ...
         if(!this.var_84)
         {
            this.var_84 = this._structure.getCanvas(this._scale,this.var_732);
            if(!this.var_84)
            {
               return null;
            }
            this.var_2320 = new Point(0,this.var_84.height - int(this.var_84.width / 4));
         }
         var _loc5_:int = param1;
         var _loc6_:Boolean = AvatarDirectionAngle.const_1735[param1];
         var _loc9_:String = param4.definition.assetPartDefinition;
         var _loc10_:String = param4.definition.state;
         var _loc11_:Boolean = _loc10_ == AvatarAction.const_896 || _loc10_ == AvatarAction.const_372;
         ...
         while(_loc8_ >= 0)
         {
            ...
            {
               this.var_1378 = this.var_116.bodyPartId;
               this.var_182 = this.var_116.partType;
               this.var_1682 = this.var_116.partId;
               ...
               if(_loc6_)
               {
                  ...
                  else
                  {
                     ...
                     if(this.var_116.flippedPartType != this.var_182)
                     {
                        this.var_182 = this.var_116.flippedPartType;
                     }
                  }
               }
               this.var_1377 = this._scale + "_" + this.var_116.action.assetPartDefinition + "_" + this.var_182 + "_" + this.var_1682 + "_" + _loc5_ + "_" + this._frame;
               this.var_428 = this._assets.getAssetByName(this.var_1377) as BitmapDataAsset;
               if(this.var_428 == null)
               {
                  this.var_1377 = this._scale + "_std_" + this.var_182 + "_" + this.var_1682 + "_" + _loc5_ + "_0";
                  this.var_428 = this._assets.getAssetByName(this.var_1377) as BitmapDataAsset;
               }
               if(!(!this.var_428 || !this.var_428.content))
               {
                  if(this.var_116.paletteMapId > -1)
                  {
                     this.var_995 = this._structure.renderManager.paletteManager.getPalette(this.var_116.paletteMapId);
                     if(this.var_995 != null)
                     {
                        this.var_898 = this.var_428.content as BitmapData;
                        this.var_183 = new BitmapData(this.var_898.width,this.var_898.height);
                        this.var_2318 = this.var_995.colorize(this.var_898.getPixels(this.var_898.rect));
                        this.var_183.setPixels(this.var_183.rect,this.var_2318);
                     }
                     else
                     {
                        this.var_183 = (this.var_428.content as BitmapData).clone();
                     }
                  }
                  else
                  {
                  ...
                  if(_loc11_ && (this.var_1378 == AvatarBodyPartType.const_250 || this.var_1378 == AvatarBodyPartType.const_1727 || this.var_1378 == AvatarBodyPartType.const_1724) && this.var_116.color != null)
                  {
                     _loc16_ = 0.7;
                     ...
                     this.var_183.colorTransform(this.var_183.rect,_loc18_);
                  }
                  else if(this.var_116.isColorable && this.var_116.color != null)
                  {
                     this.var_183.colorTransform(this.var_183.rect,this.var_116.color.colorTransform);
                  }
                  if(this.var_116.isBlendable)
                  {
                     this.var_183.colorTransform(this.var_183.rect,this.var_116.blendTransform);
                  }
                  this.var_899 = this.var_428.offset.clone();
                  if(_loc7_)
                  {
                     if(this._scale == AvatarScaleType.const_53)
                     {
                  ...
                  this.var_2319 = new ImageData(this.var_183,this.var_899,_loc7_);
                  this.var_900.push(this.var_2319);
               }
            }
            _loc8_--;
         }
         ...
         var _loc13_:ImageData = this.createUnionImage(this.var_900,_loc6_);
         var _loc14_:Point = new Point(-1 * _loc13_.regpoint.x,this.var_2320.y - _loc13_.regpoint.y);
         if(_loc6_)
         {
            if(this._scale == AvatarScaleType.const_53)
            {
         ...
         _loc8_ = this.var_900.length - 1;
         while(_loc8_ >= 0)
         {
            _loc15_ = this.var_900.pop();
            if(_loc15_)
            {
               _loc15_.dispose();
            }
