package com.sulake.habbo.avatar
{
   ...
   public class AvatarAssetDownloadManager extends EventDispatcher
   {
      ...
      private var var_873:int = 3;
      
      private var var_414:AssetLoaderStruct;
      
      private var var_872:Timer;
      ...
      private const const_2043:int = 100;
      ...
      public function AvatarAssetDownloadManager(param1:IAssetLibrary, param2:String, param3:String, param4:AvatarStructure)
      {
         ...
         this.var_872 = new Timer(this.const_2043,1);
         this.var_872.addEventListener(TimerEvent.TIMER_COMPLETE,this.onNextDownloadTimeout);
      }
      ...
      private function addMapLoaderEventListeners() : void
      {
         if(this.var_414)
         {
            ...
            this.var_414.addEventListener(AssetLoaderEvent.const_48,this.onConfigurationError);
         }
      }
      ...
      private function removeMapLoaderEventListeners() : void
      {
         if(this.var_414)
         {
            ...
            this.var_414.removeEventListener(AssetLoaderEvent.const_48,this.onConfigurationError);
         }
      }
      ...
      private function onConfigurationError(param1:Event) : void
      {
         ...
         {
            Logger.log("Could not download figuremap.xml. Retries left: " + this.var_873);
            if(this.var_1337.indexOf("?") > 0)
            {
               _loc2_ = this.var_1337 + "&retry=" + this.var_873;
            }
            else
            {
               _loc2_ = this.var_1337 + "?retry=" + this.var_873;
      ...
      private function loadFigureMapData(param1:XML) : void
      {
         ...
         this.var_1597 = true;
         for each(_loc2_ in this.var_1065)
         {
      ...
      private function generateMap(param1:XML) : void
      {
         ...
         {
            _loc3_ = new AvatarAssetDownloadLibrary(_loc2_.@id,_loc2_.@revision,this.var_2158,this._assets);
            _loc3_.addEventListener(Event.COMPLETE,this.libraryComplete);
            for each(_loc4_ in _loc2_.part)
            {
               _loc5_ = _loc4_.@type + ":" + _loc4_.@id;
               _loc6_ = this.var_1066[_loc5_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(_loc3_);
               this.var_1066[_loc5_] = _loc6_;
            }
         }
      }
      ...
      public function loadFigureSetData(param1:IAvatarFigureContainer, param2:IAvatarImageListener) : void
      {
         ...
         if(!this.var_1597)
         {
            this.var_1065.push([param1,param2]);
            return;
         }
      ...
      private function libraryComplete(param1:Event) : void
      {
         ...
         var _loc10_:* = null;
         var _loc2_:* = [];
         for(_loc3_ in this.var_874)
         {
            _loc7_ = true;
            _loc4_ = this.var_874[_loc3_];
      ...
      {
         var _loc1_:* = null;
         while(this.var_703.length > 0 && this.var_702.length < this.const_1726)
         {
            _loc1_ = this.var_703[0];
            _loc1_.startDownloading();
            this.var_702.push(this.var_703.shift());
         }
      }
      
      private function addToQueue(param1:AvatarAssetDownloadLibrary) : void
      {
         if(!param1.isReady && this.var_703.indexOf(param1) == -1 && this.var_702.indexOf(param1) == -1)
         {
