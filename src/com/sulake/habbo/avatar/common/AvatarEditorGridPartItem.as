package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridPartItem implements IAvatarImageListener
   {
      ...
      private var var_1401:IWindow;
      
      private var var_745:IFigurePartSet;
      ...
      private var var_605:Rectangle;
      
      private var var_1728:int = 0;
      
      private var var_434:IAvatarRenderManager;
      
      private var var_820:Boolean;
      
      private var var_1727:BitmapData;
      
      private const const_2047:int = 13421772;
      
      private const const_2048:int = 16777215;
      
      private const const_342:Array = [2,6,0,4,3,1];
      ...
      public function AvatarEditorGridPartItem(param1:IWindowContainer, param2:IAvatarEditorCategoryModel, param3:IFigurePartSet, param4:Array, param5:Boolean = true)
      {
         ...
         this.var_745 = param3;
         this._window = param1;
         this.var_1401 = this._window.findChildByTag("BG_COLOR");
         this._colors = param4;
         this.var_1729 = param5;
         if(param3 == null)
         {
            this.var_689 = new BitmapData(1,1,true,16777215);
         }
         if(param3 != null)
         {
            _loc7_ = param3.parts;
         ...
         this.var_434 = this.var_67.controller.avatarRenderManager;
      ...
      private function updateThumbVisualization() : void
      {
         ...
         if(_loc1_)
         {
            if(this.var_689 != null && !this.var_1729)
            {
               _loc3_ = this.var_689;
            }
            else
         ...
         if(_loc2_)
         {
            if(this.var_745)
            {
               switch(this.var_745.clubLevel)
               {
                  case HabboClubLevelEnum.const_56:
                     _loc2_.visible = false;
                     break;
                  case HabboClubLevelEnum.const_44:
                     _loc2_.style = HabboIconType.const_805;
                     break;
                  case HabboClubLevelEnum.const_46:
                     _loc2_.style = HabboIconType.const_703;
               }
            }
            else
            {
         ...
         if(this.isSelected)
         {
            this.var_1401.color = this.const_2047;
         }
         else
         {
            this.var_1401.color = this.const_2048;
         }
      ...
      private function analyzePartLayers() : Boolean
      {
         ...
         var _loc1_:IAvatarFigureContainer = this.var_434.createFigureContainer(this.partSet.type + "-" + this.partSet.id);
         Logger.log("QUERYING SET:" + this.partSet.type + "-" + this.partSet.id);
         if(!this.var_434.isFigureReady(_loc1_))
         {
            this.var_434.downloadFigure(_loc1_,this);
            return false;
         }
         var _loc6_:int = 0;
         ...
         for each(_loc2_ in this.partSet.parts)
         {
            if(_loc7_)
            {
               _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_342[_loc6_] + "_" + FigureData.const_131;
               _loc4_ = this.var_434.getAssetByName(_loc3_) as BitmapDataAsset;
            }
            else
            {
               _loc6_ = 0;
               while(!_loc7_ && _loc6_ < this.const_342.length)
               {
                  _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_342[_loc6_] + "_" + FigureData.const_131;
                  _loc4_ = this.var_434.getAssetByName(_loc3_) as BitmapDataAsset;
      ...
      private function renderThumb() : BitmapData
      {
         ...
         _loc1_ = new BitmapData(this.var_605.width,this.var_605.height,true,16777215);
         var _loc6_:int = 0;
         var _loc7_:Boolean = false;
         ...
         for each(_loc2_ in _loc8_)
         {
            ...
            if(_loc4_)
            {
               ...
               _loc10_ = -1 * _loc4_.offset.x - this.var_605.x;
               _loc11_ = -1 * _loc4_.offset.y - this.var_605.y;
               if(this.var_1729 && _loc2_.colorLayerIndex > 0)
               {
